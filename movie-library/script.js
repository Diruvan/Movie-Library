...
setInterval(nextSlide, 4000); // for every 4 seconds, slides will be changed
...
form.addEventListener('submit', (e) => {
  ...
  if (!termsCheckbox.checked) {
    isValid = false;
    alert('Please accept to the Terms & Conditions.');
    return; // Stop further validation
  }

  // if all the details that entered are valid, submit the form
  if (isValid) {
    form.submit();
  }
});
...
searchInput.addEventListener('input', async () => {
  ...
  if (query) {
    const shows = await searchMovies(query);
    displayMovies(shows);
  } else {
    displayMovies([]);
  }
